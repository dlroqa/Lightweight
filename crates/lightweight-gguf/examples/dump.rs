//! Dump a GGUF file's metadata keys. `cargo run -p lightweight-gguf --example dump -- <file>`
        ...
        Ok(file) => {
            for (key, value) in file.metadata() {
                ...
                let rendered = match value {
                    lightweight_gguf::GgufValue::String(s) if s.len() > 60 => {
                        format!("String(<{} bytes>)", s.len())
                    }
                    lightweight_gguf::GgufValue::Array(summary) if summary.len <= 64 => {
