...
use lightweight_catalog::install::{AddModel, Installer};
use lightweight_gateway::jobs::{JobKind, Jobs};
...
impl Profile {
    fn new(tag: &str) -> Self {
        ...
        let path = std::env::temp_dir().join(format!(
            "hermes-mgr-{tag}-{}-{unique}-{sequence}",
            std::process::id()
        ));
...
async fn a_download_that_is_cancelled_leaves_the_catalog_untouched() {
    ...
    match outcome {
        ...
        Err(err) => {
            assert_eq!(lightweight_core::Actionable::code(&err), "cancelled");
            assert!(
