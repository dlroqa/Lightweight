...
use lightweight_system_info::{FailingMemoryProbe, FixedMemoryProbe};
...
async fn a_measurement_of_this_machine_reaches_the_estimate_on_screen() {
    ...
    let points: Vec<lightweight_bench::fit::ResidualPoint> = [n_ubatch / 2, n_ubatch, n_ubatch * 2]
        .into_iter()
        .map(|ubatch| lightweight_bench::fit::ResidualPoint {
            n_ubatch: ubatch,
            residual_bytes: (slope * f64::from(ubatch) + intercept) as u64,
            peak_rss: Bytes::ZERO,
        })
        .collect();
    let mut calibration = lightweight_bench::fit::Calibration::default();
    calibration.insert(lightweight_bench::fit::Fit {
        at_unix: 1_700_000_000,
        machine: lightweight_bench::MachineFingerprint::detect(),
        engine: lightweight_bench::engine_fingerprint(&MockBackend::default()),
        bucket,
    ...
    let paths = lightweight_system_info::DataPaths::rooted_at(dir.path());
