...
use lightweight_inference::EngineCounters;
use lightweight_inference::generation::FinishReason;
use serde::Serialize;
...
/// same reason `lightweight_system_info::load` publishes `/proc/stat` unconverted —
...
/// [`lightweight_system_info::load`] for why the figures that *are* rates are not
