...
use lightweight_catalog::install::AddModel;
use lightweight_catalog::{InstalledModel, manifest};
use lightweight_core::{Actionable, GgmlType};
    ...
    tokio::spawn(async move {
        ...
        if let Err(err) = result {
            background.set(JobState::Failed {
                error: lightweight_core::ErrorReport::capture(&err),
            });
        }
    });
    accepted(&job)
...
pub struct GatewayReport {
    version: &'static str,
    backend: String,
    engine: lightweight_inference::BackendHealth,
    model: Option<String>,
    ...
    engine_capabilities: lightweight_inference::BackendCapabilities,
...
impl From<&lightweight_gguf::ModelMetadata> for HeaderDetail {
    fn from(metadata: &lightweight_gguf::ModelMetadata) -> Self {
        Self {
            ...
            context_presets: lightweight_core::RuntimeParams::context_presets_for(
...
pub struct ModelDetail {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    estimate: Option<Probed<lightweight_memory::Estimate>>,
...
fn describe_file(
    ...
    asked: DetailOptions,
    memory: &dyn lightweight_system_info::MemoryProbe,
    calibration: &crate::state::CalibrationLookup,
) -> (
    Option<HeaderDetail>,
    Option<Probed<lightweight_memory::Estimate>>,
    Option<manager::ContextSource>,
) {
    // The catalog's own reader, not a second copy: "is this a model?" must have
    // one answer.
    let Ok(metadata) = lightweight_catalog::read_header(path) else {
    ...
    let base = lightweight_core::RuntimeParams {
        cache_type_k: cache_type,
        cache_type_v: cache_type,
        threads: Some(
            defaults
                .threads
                .unwrap_or_else(|| lightweight_system_info::CpuInfo::detect().default_threads()),
        ),
        ...
        // button pays.
        n_parallel: 1,
        ..lightweight_core::RuntimeParams::default()
    };
    ...
    let estimator = lightweight_memory::Estimator::headless();
    let slots = estimator.choose_concurrency(
        defaults.concurrency,
        lightweight_system_info::CpuInfo::detect().logical_cores,
        Some(&metadata),
        base,
    ...
    GatewayReport {
        ...
        defaults: {
            ...
            DefaultsReport {
                ...
                ubatch: lightweight_core::RuntimeParams::default().n_ubatch,
                load_modes: lightweight_core::LoadMode::ALL
                    .iter()
...
fn describe_calibration(state: &GatewayState) -> CalibrationReport {
    ...
    match lightweight_bench::fit::Calibration::load(&path) {
        Ok(calibration) => {
            let machine = lightweight_bench::MachineFingerprint::detect();
                ...
                match received {
                    Ok(event) => {
                        let payload = serde_json::to_string(&event).unwrap_or_else(|err| {
                            ...
                            // feed that never ran.
                            tracing::error!(
                                target: lightweight_observability::targets::API,
                                error = %err,
...
fn finish<E: Actionable>(job: &Arc<Job>, result: Result<InstalledModel, E>) {
    match result {
        ...
        Err(err) if err.kind() == lightweight_core::ErrorKind::Cancelled => {
