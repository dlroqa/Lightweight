...
use lightweight_catalog::{CatalogError, CatalogStore, InstalledModel};
use lightweight_core::{
    Actionable, ErrorKind, GgmlType, ModelId, Remedy, RemedyAction, RuntimeParams, SettingsSection,
};
use lightweight_inference::{BackendError, LoadProgress, LoadRequest};
use lightweight_memory::{Estimator, Verdict};
use lightweight_observability::targets;
use lightweight_system_info::CpuInfo;
...
pub struct LoadOptions {
    ...
    pub load_mode: Option<lightweight_core::LoadMode>,
...
pub enum ManagerError {
    ...
    /// because that type lives in `lightweight-inference`, which cannot see an
    ...
    Insufficient {
        ...
        required: String,
        available: String,
        estimate: Box<lightweight_memory::Estimate>,
    },
    ...
    #[error(transparent)]
    MemoryProbe(#[from] lightweight_system_info::MemoryError),
}
...
fn describe_calibration(outcome: &lightweight_bench::Outcome) -> String {
    match outcome {
        lightweight_bench::Outcome::Applied(calibrated) => {
            format!("measured (fitted at {})", calibrated.fit_at_unix)
        }
        lightweight_bench::Outcome::Rejected(untrusted) => {
            format!("shipped defaults: {}", untrusted.reason())
        }
        lightweight_bench::Outcome::Unreadable(detail) => {
...
pub fn choose_context(
    ...
    base: RuntimeParams,
    budget: lightweight_memory::Budget,
    estimator: &Estimator,
) -> ContextChoice {
...
pub async fn load_model(
    ...
    // have one answer.
    let metadata = lightweight_catalog::read_header(&model.path)?;
    ...
    let mut judged: Option<lightweight_memory::Estimate> = None;
    ...
    if let Some(snapshot) = snapshot {
        let reclaimable = if state.catalog.resident().await.is_some() {
            ...
            let anonymous = usage
                ...
                .unwrap_or(lightweight_core::units::Bytes::ZERO);
            let locked = state
                .catalog
                .resident()
                .await
                .filter(|resident| {
                    resident
                        .effective
                        .load_mode
                        .is_some_and(lightweight_core::LoadMode::locks_weights)
                })
                .and_then(|_| usage.map(|usage| usage.rss.get().saturating_sub(anonymous.get())))
                .unwrap_or(0);
            lightweight_core::units::Bytes(anonymous.get().saturating_add(locked))
        } else {
            lightweight_core::units::Bytes::ZERO
        };
        let budget = lightweight_memory::Budget::of(snapshot).reclaiming(reclaimable);
...
mod tests {
    ...
    fn a_machine_that_could_not_be_measured_says_so_and_offers_a_way_through() {
        ...
        let err = ManagerError::from(lightweight_system_info::MemoryError::UnsupportedPlatform {
            platform: "macos",
        });
