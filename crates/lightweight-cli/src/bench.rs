...
use lightweight_backend_llamacpp::manifest::PINNED_BUILD;
use lightweight_bench::fit::{Calibration, fit_run};
use lightweight_bench::record::{
    BenchmarkRun, EngineFingerprint, MachineFingerprint, ModelFingerprint, Prediction, Sample,
};
use lightweight_bench::{BenchmarkStore, RunPlan, Runner};
use lightweight_core::{GgmlType, ModelId, RuntimeParams};
use lightweight_gguf::{GgufFile, ModelMetadata};
use lightweight_inference::{InferenceBackend, LoadRequest};
use lightweight_memory::Estimator;
use lightweight_system_info::{CpuInfo, DataPaths, MemoryProbe, SystemMemoryProbe};
...
pub async fn run(options: &BenchOptions) -> Result<String, String> {
    ...
    for (index, (slots, ubatch)) in buckets.iter().enumerate() {
        ...
        let mut scenarios = lightweight_bench::record::Scenario::ALL.to_vec();
        if *slots > 1 {
            scenarios.push(lightweight_bench::record::Scenario::ConcurrentDecode);
        }
        let plan = RunPlan {
...
fn render_json(run: &BenchmarkRun, fits: Option<&[lightweight_bench::fit::Fit]>) -> String {
...
fn render_human(
    run: &BenchmarkRun,
    path: &Path,
    fitted: Option<&(PathBuf, Vec<lightweight_bench::fit::Fit>)>,
    metadata: &ModelMetadata,
) -> String {
    ...
    if run
        .samples
        .iter()
        .any(|sample| sample.scenario == lightweight_bench::record::Scenario::ConcurrentDecode)
    {
        scenarios.push(lightweight_bench::record::Scenario::ConcurrentDecode);
    }
    ...
    for scenario in scenarios {
        ...
        for sample in samples {
            ...
            let rate = match scenario {
                lightweight_bench::record::Scenario::Decode
                | lightweight_bench::record::Scenario::ConcurrentDecode => format!(
                    "decode {:>10}",
        ...
        .max()
    {
        ...
        if let Some(prediction) = run.samples.iter().find_map(|sample| sample.predicted) {
            let _ = writeln!(
                ...
                prediction.total,
                lightweight_core::units::Bytes(prediction.exact())
            );
        }
    }
            ...
            match (fit.compute_bytes_per_ubatch, fit.overhead_bytes) {
                (Some(slope), Some(intercept)) => {
                    let _ = writeln!(
                        ...
                        slope,
                        lightweight_core::units::Bytes(intercept.max(0.0) as u64),
                        match fit.r_squared {
                ...
                _ => {
                    let _ = writeln!(
                        ...
                        fit.bucket.architecture,
                        fit.bucket.quantization,
                        lightweight_core::units::Bytes(fit.max_residual_bytes)
                    );
                }
            }
