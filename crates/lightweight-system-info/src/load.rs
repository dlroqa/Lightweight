...
mod platform {
    ...
    /// The same counters `/proc/stat` gives, read through the one FFI crate.
    ///
    /// macOS (`host_statistics`) and Windows (`GetSystemTimes`) both publish a
    /// running total of processor time summed over every core, and
    /// `lightweight-sys` normalises both to the ticks `/proc/stat` uses, so the
    /// value that lands here means what the Linux branch's does and the same
    /// caller differences it the same way. A probe failure is reported rather
    /// than papered over, for the reason [`crate::memory`] gives: a utilization
    /// tile is display-only, so it goes blank rather than showing a made-up
    /// number.
    pub(super) fn cpu_times() -> Result<CpuTimes, LoadError> {
        let load = lightweight_sys::cpu_load::read().map_err(|error| LoadError::Read {
...
";
    ...
    // The real-probe tests run on every platform the probe is implemented for,
    // not just the one it was first written on: a probe proves it reads this
    // machine only on the machine it ran on, and the macOS and Windows paths go
    // through FFI that a Linux run cannot exercise. CI runs the suite on all
    // three, which is where these turn from assertions into evidence.
