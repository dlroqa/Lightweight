...
use lightweight_core::{Actionable, Bytes, ErrorKind, Remedy, RemedyAction, SettingsSection};
...
mod platform {
    ...
    pub(super) fn space_for(path: &Path) -> Result<DiskSpace, DiskError> {
        let raw = lightweight_sys::disk::space_for(path).map_err(|error| DiskError::Query {
            path: path.to_path_buf(),
            source: error.source,
        })?;
        Ok(DiskSpace {
