...
//! [`lightweight_core::Private`], which redacts through `Display` and `Debug` and so
...
use lightweight_core::privacy::{self, PrivacyMode};
...
/// `RUST_LOG` syntax (`info`, `lightweight_gateway=debug,hyper=warn`, ...).
    ...
    fn remedies(&self) -> Vec<Remedy> {
        match self {
            ...
            Self::Filter { .. } => vec![Remedy::new(
                format!("Correct {FILTER_ENV}, for example `info` or `lightweight_gateway=debug`"),
                RemedyAction::OpenSettings {
