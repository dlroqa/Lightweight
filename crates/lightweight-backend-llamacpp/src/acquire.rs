...
//!
//! That transfer now lives in [`lightweight_download`], because the model catalog
...
use lightweight_download::{DownloadError, Fetch};
use lightweight_inference::{BackendError, LoadProgress};
...
impl RuntimeInstaller {
    ...
    ///
    /// The transfer itself is [`lightweight_download::fetch`]; what is added here is
...
mod tests {
    ...
    // Resuming a partial download is exercised in `lightweight-download`, which
    // owns that code now.
