...
use lightweight_core::{InstanceId, ModelId, RuntimeParams, units::Bytes};
use lightweight_gguf::architecture;
use lightweight_inference::generation::GenerationRequest;
use lightweight_inference::{
...
use lightweight_inference::CpuTicks;
    ...
    fn capabilities(&self) -> BackendCapabilities {
        BackendCapabilities {
            ...
            kv_cache_types: ALLOWED_KV_CACHE_TYPES
                .iter()
                .filter_map(|name| lightweight_core::GgmlType::from_name(name))
                .collect(),
        }
    }
...
fn parse_process_status(status: &str) -> Option<ResourceSnapshot> {
    ...
    Some(ResourceSnapshot {
        ...
        peak_kind: lightweight_inference::PeakKind::ResidentSet,
...
/// `lightweight-system-info` on top of the workspace's only FFI crate - so this is
...
fn read_process_usage(pid: u32) -> Option<ResourceSnapshot> {
    let usage = lightweight_system_info::process::usage(pid).ok()?;
    Some(ResourceSnapshot {
        rss: usage.rss,
        peak_rss: usage.peak_rss,
        peak_kind: if usage.peak_is_footprint {
            lightweight_inference::PeakKind::Footprint
        } else {
            lightweight_inference::PeakKind::ResidentSet
        },
