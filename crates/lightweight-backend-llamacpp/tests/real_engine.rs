...
use lightweight_core::{Actionable, GgmlType, ModelId, RuntimeParams};
use lightweight_gguf::{GgufFile, ModelMetadata};
use lightweight_inference::generation::{
    ChatMessage, FinishReason, GenerationEvent, GenerationRequest, ReasoningControl,
    SamplingParams, ToolChoice, ToolDefinition,
};
use lightweight_inference::{BackendHealth, InferenceBackend, LoadRequest};
use lightweight_system_info::MemoryProbe as _;
use tokio::sync::mpsc;
...
impl Profile {
    fn new(tag: &str) -> Self {
        ...
        let path = std::env::temp_dir().join(format!(
            "hermes-real-{tag}-{}-{unique}-{sequence}",
            std::process::id()
        ));
...
async fn the_real_engine_loads_a_real_model_and_reports_its_memory() {
    ...
    assert!(
        usage.rss > lightweight_core::Bytes::from_mib(64),
    ...
    match usage.peak_kind {
        lightweight_inference::PeakKind::ResidentSet => assert!(usage.peak_rss >= usage.rss),
        lightweight_inference::PeakKind::Footprint => assert!(
...
async fn the_ram_estimate_is_an_upper_bound_on_what_the_engine_actually_uses() {
    ...
    let estimate = lightweight_memory::Estimator::headless().estimate(
        &metadata,
        params,
