...
use lightweight_backend_llamacpp::supervisor::{self, EngineConfig, ExitClassification};
use lightweight_core::{Actionable, RuntimeParams};
...
async fn an_engine_that_exits_during_startup_reports_why() {
    ...
    match err {
        lightweight_inference::BackendError::EngineCrashed {
            exit_code, tail, ..
        } => {
