...
use lightweight_bench::record::Scenario;
use lightweight_bench::runner::{RunPlan, Runner};
use lightweight_core::RuntimeParams;
use lightweight_gguf::metadata::{ModelMetadata, QuantMix, TokenizerMeta};
...
async fn loaded(config: MockConfig) -> (MockBackend, lightweight_core::InstanceId) {
    let backend = MockBackend::new(config);
    let request = LoadRequest {
        model: lightweight_core::ModelId::with_context("mock-model", 4096),
