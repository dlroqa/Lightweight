...
use hermes_catalog::record::Integrity;
use hermes_catalog::{CatalogError, CatalogStore};
...
async fn a_pinned_model_downloads_and_verifies_against_its_recorded_digest() {
    ...
    let expected = hermes_catalog::manifest::by_id(PINNED).expect("pinned entry");
    assert_eq!(
...
async fn an_interrupted_download_resumes_from_what_is_already_on_disk() {
    ...
    let entry = hermes_catalog::manifest::by_id(PINNED).expect("pinned entry");
    ...
    let second = hermes_download::fetch(
        &client,
        hermes_download::Fetch {
            url: &entry.url(),
            destination: &destination,
...
async fn a_digest_that_does_not_match_is_refused_and_the_bytes_are_discarded() {
    ...
    assert!(
        !hermes_download::partial_path(&destination).exists(),
