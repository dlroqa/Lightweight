...
# ---------------------------------------------------------------------------
echo "== cli archive =="
archive="$(find dist-cli -maxdepth 1 \( -name 'lightweight-*.tar.gz' -o -name 'lightweight-*.zip' \) -print -quit 2>/dev/null || true)"
if [ -z "$archive" ]; then
  ...
  binary="$(find "$work" -name 'lightweight' -o -name 'lightweight.exe' | head -1)"
...
Darwin)
  ...
  if [ -z "$dmg" ]; then
    ...
    if [ -z "$app" ]; then
      ...
      for executable in lightweight lightagent; do
        staged="$app/Contents/Resources/bin/$executable"
        if [ -f "$staged" ]; then
          info="$(lipo -info "$staged" 2>&1 || true)"
          echo "        $info"
          case "$info" in
            *arm64*) pass "the bundled $executable has an arm64 slice" ;;
            *) fail "no arm64 slice: $info" ;;
          esac
          case "$info" in
            *x86_64*) pass "the bundled $executable has an x86_64 slice" ;;
            *) fail "no x86_64 slice: $info" ;;
          esac
          "$staged" --version >/dev/null 2>&1 \
            && pass "the bundled $executable runs on this machine ($(uname -m))" \
            || fail "the bundled $executable does not run"
        else
          fail "the app bundle carries no $executable binary"
        fi
      done
...
Linux)
  ...
  binary="target/${triple:-none}/release/lightweight"
  [ -f "$binary" ] || binary="target/release/lightweight"
  if [ -f "$binary" ]; then
...
MINGW* | MSYS* | CYGWIN* | Windows_NT)
  ...
  if [ -z "$setup" ]; then
    ...
    # Wait for the *bundled* binary, `resources/bin/lightweight.exe`, not the
    ...
    # and the `-f` on `resources/bin/lightweight.exe` failed in the same millisecond,
    # reported as "the installed lightweight does not run". The file the run
    # assertion below actually needs is the one the loop must wait for.
    for _ in $(seq 1 180); do
      [ -f "$target/resources/bin/lightweight.exe" ] && break
      [ -f "$fallback/resources/bin/lightweight.exe" ] && break
      sleep 1
    done
    if [ -f "$target/resources/bin/lightweight.exe" ]; then
      pass "the installer wrote an application"
    elif [ -f "$fallback/resources/bin/lightweight.exe" ]; then
      pass "the installer wrote an application (at its default location)"
      target="$fallback"
    else
      fail "the installer produced no resources/bin/lightweight.exe"
    ...
    installed="$target/resources/bin/lightweight.exe"
    if [ -f "$installed" ] && "$installed" --version >/dev/null 2>&1; then
      pass "the installed lightweight runs ($("$installed" --version))"
    else
      fail "the installed lightweight does not run"
    fi
    agent_installed="$target/resources/bin/lightagent.exe"
    if [ -f "$agent_installed" ] && "$agent_installed" --version >/dev/null 2>&1; then
      pass "the installed lightagent runs"
    else
      fail "the installed lightagent does not run"
    fi
    ...
    taskkill //F //IM lightweight.exe >/dev/null 2>&1 || true
