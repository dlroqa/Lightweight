...
describe("the sandbox verdict", () => {
  it("accepts an ordinary Linux launch", () => {
    const verdict = inspectSandbox(["/opt/Hermes/lightweight", "%U"], "linux");
    assert.equal(verdict.sandboxed, true);
  });

  it("refuses a Linux launch that disabled the sandbox", () => {
    const verdict = inspectSandbox(["/opt/Hermes/lightweight", "--no-sandbox"], "linux");
  ...
  it("refuses the assignment form too", () => {
    ...
    const verdict = inspectSandbox(["lightweight", "--no-sandbox=1"], "linux");
    assert.equal(verdict.sandboxed, false);
  });

  it("names both ways out, and neither of them is 'ignore this'", () => {
    const text = sandboxFailureText(inspectSandbox(["lightweight", "--no-sandbox"], "linux"));
  ...
  it("does not judge macOS or Windows", () => {
    ...
    for (const platform of ["darwin", "win32"]) {
      const verdict = inspectSandbox(["lightweight", "--no-sandbox"], platform);
