/**
 * Put the gateway and agent binaries the installer will carry where electron-builder can
 * find them, and refuse missing or mismatched versions.
 *
 * Four problems this solves, all of which were live before it existed:
 *
 * 1. **The stale binary.** `extraResources` pointed straight at
 *    `target/release/lightweight`, so whatever happened to be there got shipped. A
 ...
 *    shell's own resolver looks for `bin/lightweight.exe` on Windows - so a Windows
 ...
 *    them unless they are declared in `mac.x64ArchFiles`. `lightweight` differs per
...
const BINARIES = ["lightweight", "lightagent"];
  ...
  if (found !== version) {
    die(
      `${binary} reports ${found}, but this package is ${version}.\n` +
        "       Rebuild it: cargo build --release -p lightweight-cli -p lightagent",
    );
  }
...
function stageMacUniversal() {
  for (const target of MAC_TARGETS) {
    ...
    run("cargo", ["build", "--release", "-p", "lightweight-cli", "-p", "lightagent", "--target", target], {
      cwd: repoRoot,
      stdio: "inherit",
    });
  }
  for (const executable of BINARIES) {
    const slices = MAC_TARGETS.map((target) => join(repoRoot, "target", target, "release", executable));
    const merged = join(stagingBin, executable);
    run("lipo", ["-create", "-output", merged, ...slices]);
    const info = run("lipo", ["-info", merged]).trim();
    for (const architecture of ["arm64", "x86_64"]) {
      if (!info.includes(architecture)) {
        die(`${executable} is missing the ${architecture} slice: ${info}`);
      }
    }
  ...
  if (!found) {
    die(
      `no binary at ${candidates.join(" or ")}\n` +
        "       Build it first: cargo build --release -p lightweight-cli -p lightagent",
    );
  }
  try {
...
if (process.platform === "darwin") {
  ...
  for (const name of BINARIES) stageHost(name);
}
